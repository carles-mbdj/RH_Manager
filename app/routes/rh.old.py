...
from app.models import Employee, Absence, Conge, Recrutement
from app.forms import EmployeeForm, AbsenceForm, CongeForm, RecrutementForm
...
@rh_bp.route('/recrutement')
def list_recrutement():
    recrutements = Recrutement.query.all()
    form = RecrutementForm()
    return render_template('recrutement/list.html', recrutements=recrutements, form=form)

@rh_bp.route('/recrutement/add', methods=['POST'])
def add_recrutement():
    form = RecrutementForm()
    if form.validate_on_submit():
        recrutement = Recrutement(
            ...
            departement=form.departement.data,
            description_poste=form.description_poste.data,
            prerequis=form.prerequis.data,
            date_cloture=form.date_cloture.data,
            statut=form.statut.data
        )
        db.session.add(recrutement)
        db.session.commit()
        flash("Le recrutement a été ajouté avec succès.", "success")
    else:
        flash("Erreur lors de l'ajout du recrutement. Vérifiez le formulaire.", "danger")

    return redirect(url_for('rh.list_recrutement'))
...
@rh_bp.route('/recrutement/edit/<int:id>', methods=['POST'])
def edit_recrutement(id):
    rec = Recrutement.query.get_or_404(id)
    form = RecrutementForm()

    if form.validate_on_submit():
        rec.titre_poste = form.titre_poste.data
        rec.departement = form.departement.data
        rec.description_poste = form.description_poste.data
        rec.prerequis = form.prerequis.data
        rec.date_cloture = form.date_cloture.data
        rec.statut = form.statut.data

        db.session.commit()
        flash("L'offre de recrutement a été modifiée avec succès.", "success")
        return redirect(url_for('rh.list_recrutement'))
...
@rh_bp.route('/recrutement/delete/<int:id>', methods=['POST'])
def delete_recrutement(id):
    rec = Recrutement.query.get_or_404(id)
    db.session.delete(rec)
    db.session.commit()
    flash("L'offre de recrutement a été supprimée avec succès.", "success")
    return redirect(url_for('rh.list_recrutement'))
